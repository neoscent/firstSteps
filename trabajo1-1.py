...
# 9.6.20
# Trabajo 1: (a) Mayor Matriz Rectangular Nula
        ...
        for y in range (0, len(A)):
            ...
            if y >= len(A)-1 or A[y][1] < x:
                if countElem*x > currentMax[0]*currentMax[1]: # Dependiendo de la funcion de la cual searchMax()
                    currentMax[0] = x                         # haya sido llamada, countElem y 'x' corresponden a filas y columnas o viceversa.
                    currentMax[1] = countElem
                    currentMax[2] = temp_y
                countElem = 0
            y+=1
    return currentMax
...
def greaterHeight(M):
    height_m, width_n = 0 , 0       # Height = Filas, Width = Columnas
    A=[]
            ...
            if M[i][j] == 0:        # Si el elemento es igual a 0
                while i < len(M) and M[i][j] == 0:  #Se cuentan los 0 adyacentes, dentro de la misma columna
                    ...
                    if maxZero < countCol:
                        maxZero = countCol
                    i+=1
            temp_w, temp_h, temp_i = searchMax(A, maxZero)
            while len(A)>0: # Se vacia la lista
                A.pop()
            if (temp_h*temp_w) > (height_m*width_n):   # Si la matriz encontrada tiene mayores dimensiones...
                height_m = temp_h
                width_n = temp_w
                x = temp_i
                y = j
    tuple = (x, y, height_m, width_n)
    return tuple
            ...
            if M[i][j] == 0:        # Si el elemento es igual a 0
                while j < len(M[0]) and M[i][j] == 0: #Se cuentan los 0 adyacentes, dentro de la misma fila
                    ...
                    if maxZero < countRow:
                        maxZero = countRow
                    j+=1
            temp_h, temp_w, temp_x = searchMax(A, maxZero)
            while len(A)>0: # Se vacia la lista
                A.pop()
            if (temp_h*temp_w) > (height_m*width_n): # Si la matriz encontrada tiene mayores dimensiones...
                height_m = temp_h
